...
> **说明**：订单构件采用DDD四层架构设计，包含接口层（OrderController）、应用层（OrderApplicationService）、领域层（Order聚合根、OrderItem实体、OrderStatus值对象）、基础设施层（OrderRepositoryImpl、OrderMapper）以及外部服务网关和消息队列。
...
**DDD四层架构**：
- 接口层：REST API接口、DTO、Assembler
- 应用层：用例编排、事务管理
- 领域层：聚合根、实体、值对象、领域服务
- 基础设施层：仓储实现、Mapper、外部网关
- 领域层：实体类和值对象
- 数据访问层：Repository模式
