...
| 404 | PaymentNotFoundException | 支付订单不存在 |
| 404 | CartItemNotFoundException | 购物车项不存在 |
| 404 | AddressNotFoundException | 收货地址不存在 |

**业务异常类（4xxx）**：

| 错误码 | 异常类 | 说明 |
|-------|-------|------|
| 4001 | InsufficientStockException | 库存不足 |
| 4002 | OrderStatusException | 订单状态异常（不允许的状态转换） |
| 4004 | OrderExpiredException | 订单已过期 |

**系统异常类（5xxx）**：

| 错误码 | 异常类 | 说明 |
|-------|-------|------|
| 5001 | StockLockFailedException | 库存锁定失败（乐观锁冲突） |
| 5002 | PaymentGatewayException | 支付网关异常 |
