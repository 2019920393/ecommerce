...
基于[`11.体系结构决策描述文档.md`](11.体系结构决策描述文档.md)中的决策1和决策2，订单服务和支付服务采用**领域驱动设计（DDD）四层架构**。

#### 2.1.1 架构风格图

**PlantUML源文件**：[`类图UML/ArchitectureStyle_DDD.puml`](类图UML/ArchitectureStyle_DDD.puml)

**架构风格图可视化**：

![架构风格图](out/类图UML/ArchitectureStyle_DDD/订单服务与支付服务架构风格图_DDD四层架构.png)

#### 2.1.2 DDD四层架构说明

**四层架构**：
1. **接口层（Interfaces Layer）**
   - 职责：处理HTTP请求、参数验证、DTO转换、响应封装
   - 组件：Controller、DTO、Assembler
   - 技术：Spring MVC、RESTful API

2. **应用层（Application Layer）**
   - 职责：用例编排、事务管理、调用领域服务、协调外部服务
   - 组件：ApplicationService、Event Handler
   - 技术：Spring Service、OpenFeign

3. **领域层（Domain Layer）**
   - 职责：核心业务逻辑、领域模型、业务规则
   - 组件：聚合根（Aggregate）、实体（Entity）、值对象（Value Object）、领域服务（Domain Service）、仓储接口（Repository Interface）
   - 技术：纯Java POJO，不依赖框架

4. **基础设施层（Infrastructure Layer）**
   - 职责：技术实现、数据持久化、外部服务集成
   - 组件：仓储实现（RepositoryImpl）、Mapper、Gateway、Config
   - 技术：MyBatis-Plus、RabbitMQ、Redis

#### 2.1.3 DDD核心概念

- **聚合根（Aggregate Root）**：领域模型的入口，保证聚合内的一致性（如Order、PaymentOrder）
- **实体（Entity）**：具有唯一标识的领域对象（如OrderItem、PaymentRecord）
- **值对象（Value Object）**：无唯一标识，通过属性值定义的对象（如OrderStatus、PaymentMethod）
- **领域服务（Domain Service）**：跨聚合的业务逻辑
- **仓储（Repository）**：领域层定义接口，基础设施层实现

#### 2.1.4 架构原则

1. **依赖倒置**：领域层不依赖任何其他层，其他层依赖领域层
2. **领域核心**：业务逻辑集中在领域层，其他层为领域层服务
3. **职责分离**：每层有明确的职责边界
4. **技术无关**：领域层不依赖具体技术框架
...
├── interfaces/              # 接口层
│   ├── controller/          # REST控制器
│   ├── dto/                 # 数据传输对象（xxxRequest、xxxRspVO）
...
基于[`11.体系结构决策描述文档.md`](11.体系结构决策描述文档.md)决策1和决策2，订单构件内部采用**DDD四层架构**：

```
┌─────────────────────────────────────┐
│       接口层 (Interfaces)           │
│  - OrderController (REST API)       │
│  - CreateOrderRequest/RspVO DTO     │
│  - OrderAssembler (对象转换)        │
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
...
│  - CreatePaymentRequest/RspVO       │
│  - PaymentAssembler (对象转换)      │
└──────────────┬──────────────────────┘
               │
┌──────────────▼──────────────────────┐
│       应用层 (Application)          │
...
┌──────────────▼──────────────────────┐
│     基础设施层 (Infrastructure)     │
│  - PaymentRepositoryImpl (仓储实现) │
│  - PaymentMapper (MyBatis映射)      │
│  - 第三方支付网关适配器             │
└─────────────────────────────────────┘
```
...
- **生成的PNG图片**：`out/类图UML/ArchitectureStyle_DDD/订单服务与支付服务架构风格图_DDD四层架构.png`
